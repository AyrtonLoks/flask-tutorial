...
# Either the form is displayed, or the posted data is validated and the post is added 
# to the database or an error is shown.
@login_required
...
def get_post(id, check_author=True):
    ...
    # if the author matches the logged in user.
    post = get_db().execute(
        'SELECT p.id, title, body, created, author_id, username'
        ' FROM post p JOIN user u ON p.author_id = u.id'
        ' WHERE p.id = ?',
    ...
    # to get a post without checking the author.
        abort(403)
